...
public class SList {
  ...
  public void insertEnd(Object obj) {
    if (head == null) {
      ...
      SListNode node = head; // it is SList call this insertEnd function, not SListNode, so we have to initialize a SListNode as the very start, i.e head
  ...
  public void squish() {
    if (head == null){ // an empty SList calls squish(), return null.
      return;
    }

    SListNode prevNode = this.head;
    SListNode currNode = this.head.next;

    while(currNode!=null){
      if (prevNode.item.equals(currNode.item)){
        prevNode.next = currNode.next; // delete the SListNode with same item value
        size--;
      }
      else{
        prevNode = currNode;
  ...
  public void twin() {
    if (head == null){ // an empty SList calls twin(), return null.
      return;
    }

    SListNode currNode = this.head;
    while(currNode!=null){
      currNode.next = new SListNode(currNode.item, currNode.next); //Make new SListNodes, but just copy the references to the items.
      size++;
      currNode = currNode.next.next; //move currNode to (copy of currNode)'s next
    }
  }
