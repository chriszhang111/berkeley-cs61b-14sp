...
public class Homework3 {
  ...
  public static void smoosh(int[] ints){
    int original_Pos = 0; //moving pointer
    int res_Pos = 0; //result array's pointer

    while(original_Pos<ints.length){
      int pointVal = ints[res_Pos] = ints[original_Pos]; //Copy the current element from the original position to the target position first, this helps copy every new value elment 
      res_Pos++;
      original_Pos++; //move both pointer to next position, preparing move original position until met a different element.

      while (original_Pos<ints.length && ints[original_Pos]==pointVal){
        original_Pos++; //if moving pointer points a value that same as previously copied, move the pointer
      }
    }

    while (res_Pos< ints.length){ // let the rest of resultPosition to be -1
      ints[res_Pos] = -1;
      res_Pos++;
    }
