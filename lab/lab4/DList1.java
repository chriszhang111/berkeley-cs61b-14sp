...
public class DList1 {
   ...
   */
  public DList1(int a, int b) { //two-node DList1 示意图见 http://pan.baidu.com/s/1skrdXs5 密码gm6e ；并列一起的还有two-mode DList2示意图
    head = new DListNode1();
    head.item = a;
    tail = new DListNode1();
    tail.item = b;
   ...
   */
  public void insertFront(int i) { // follow lecture 08-2 left part's sample to understand how to constructu a DList w/out Sentinel
    if (head == null){
    	...
    	//node = head.prev; //wrong
    	//示意图见图片链接: http://pan.baidu.com/s/1c1sQyyO 密码: yrqi
    	head.prev = node; // 第一步:之前head.prev指向null, 现在要把head.prev指向新加入的node,到此步时head还是老head还没变
    	node.next = head; // 第二步: 处理node, node.next应该指向之前的head
    	head = node;//第三步: 插入的最后一步，即让老head现在变成新的head，方法就是把老head的节点换成新的节点node
    }
    size++;
  }
  ...
  public void removeFront() {
    ...
    if (size ==1){
    	...
    	// 和insertFront略有不同：因为本文件做的是double Link, 所以对于insertFront
    	//首先要建立新节点node和老节点的prev、next的互相关联关系，其次还要把老head换成新head
    	// 但是删除部分，只有两步，只要满足head和head.next节点的互相关联关系即可。
    	//示意图见图片链接: http://pan.baidu.com/s/1mhrojaC 密码: 9vru
    	head.next.prev = null;
    	head = head.next;
    	size--;
    }
