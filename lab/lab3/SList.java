...
public class SList {
  ...
  private SListNode tail; //add another SListNode tail in SList to perform functions around end of SList
  private int size;
  ...
  public SList() {
    head = null;
    tail = null; // initialize empty tail
    size = 0;
  }
  ...
  public void insertFront(Object obj) { 
  	if (head == null){
  		...
  		tail = head; //if nothing in SList, create a head and let tail is the head
  	}
  	else{
    	head = new SListNode(obj, head); // create a new node whose next field points to the previous head. Update (previous) head new obj
    }
    size++;
  }
  ...
  // public void insertEnd(Object obj) { // original insetEnd method, running a linear time
  //   if (head == null) {
  //     head = new SListNode(obj);
  //   } else {
  //     SListNode node = head; //create tmp SListNode node, assign it as head and iterate from this tmp head until very end
  ...
  //pangjac : add new insetEnd method for lab 
  public void insertEnd(Object obj){
  	if (head == null){
  		head = new SListNode(obj);
  		tail = head;
  	}
  	//In original one, we create new tmp SListNode node then iterate (see above)
  	//Since now we have a tail reference directly to last of SList, we can access tail directly
  	else{
  		//tail = new SListNode(obj,tail); //wrong
  ...
  public static void main (String[] args) {
    // Fill in your solution for Part I here.
  	SList labSList = new SList();
  	labSList.insertEnd(new Integer(6));
  	labSList.insertEnd(new Integer(9));
  	labSList.insertEnd(new Integer(12));
  	System.out.println("Here is a list after insertEnd 6, 9, 12: "
                + labSList.toString());

  	labSList.insertFront(new Integer(3));
  	labSList.insertEnd(new Integer(15));
  	System.out.println("Here is a list after insertEnd 3, 15, should be as [ 3 6 9 12 15 ] : "
                + labSList.toString());

  	// given samples from lab
    testEmpty();
